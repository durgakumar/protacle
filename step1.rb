...
#i read all the sequences and load them in memory: it's simple, efficient and requires
#not much memory
...
proteins.each do |prot_seq_name|
  #i create a new thread for every sequence
  threads << Thread.new(prot_seq_name) do |prot_seq_name|
    ...
    #every 4 threads i stop creating new threads and wait for the older ones to complete
    if threads.size == 4
